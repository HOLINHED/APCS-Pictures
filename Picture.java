...
public class Picture extends SimplePicture 
{
  ...
  public void fixWater() {
    ...
    for (Pixel[] rowArray : pixels)
    {
      ...
      {
        //pixelObj.setRed(255 - pixelObj.getRed());
        pixelObj.setRed((int)(pixelObj.getRed() * 1.50) + 15);
        pixelObj.setGreen((int)(pixelObj.getGreen() * 0.5));
        pixelObj.setBlue((int)(pixelObj.getBlue() * 0.55));
      }
    }
  }
    ...
    for (int y = 0; y < pixels.length; y++) {
      for (int x = 0; x < pixels[0].length; x++){
         if (x == y) {
            ...
            for (int i = 0; i < x; i++) {
               leftPixel  = pixels[i][x];
               rightPixel = pixels[y][i];
    ...
    public void mirrorArms()
  {
    ...
    Pixel rightPixel = null;
    int width = pixels[0].length;
    for (int row = 105; row < (pixels.length/2) - 5; row++)
    {
      for (int col = 98; col < 171; col++)
      {
        leftPixel = pixels[pixels.length - 1 - row][col];
        // on same side
        rightPixel = pixels[row + 85][col];
        rightPixel.setColor(leftPixel.getColor());
      }
    }// end right arm 
    for (int row = 150; row < 233; row++){
      for (int col = 0; col < 169; col++){
         leftPixel   = pixels[row][col];
         try {
            rightPixel = pixels[row][width - 1 - col + 6]; 
  ...
  public void mirrorGull()
  {
    ...
    // loop through the rows
    for (int row = 190; row < pixels.length; row++)
    {
      // loop from 13 to just before the mirror point
      for (int col = 13; col < mirrorPoint; col++)
      {
  ...
  public int getAverageForColumn(int col) {
    ...
    Pixel[][] matrix = getPixels2D();
    int sum = 0;
   
    for (int i = 0; i < matrix.length; i++) {
       final int s = (matrix[i][col].getRed() + matrix[i][col].getGreen()
                     + matrix[i][col].getBlue()) / 3;
       sum += s;
    }
